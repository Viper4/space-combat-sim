Shader "Custom/Atmospheric Scattering" 
{
    Properties
    {
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            bool SphereIntersect(float3 ro, float3 rd, out float t0, out float t1, bool isPlanet) {
                ...
                float3 pM = ro + rd * t;
                float height = sqrLength(pM - _PlanetPosition);
            ...
            bool LightMarch(float3 p1, float3 rd, float l, out float2 lightDepth) {
                ...
                for (int i = 0; i < _LightSteps; i++)
                {
                    ...
                    float height = length(p - _PlanetPosition) - _PlanetRadius;
            ...
            v2f vert (appdata v)
            {
                ...
                v.normal *= -1;
                o.normal = v.normal;
                _PlanetPosition = mul(unity_ObjectToWorld, float4(0, 0, 0, 1));
                return o;
            }
